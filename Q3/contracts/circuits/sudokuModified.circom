...
template sudoku() {
    ...
    component rpForPuzzle[9][9];
    component rpForSolution[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rpForPuzzle[i][j] = RangeProof(32);
            rpForPuzzle[i][j].range[0] <== 0;
            rpForPuzzle[i][j].range[1] <== 9;
            rpForPuzzle[i][j].in <== puzzle[i][j];
            assert(rpForPuzzle[i][j].out == 1);

            rpForSolution[i][j] = RangeProof(32);
            rpForSolution[i][j].range[0] <== 0;
            rpForSolution[i][j].range[1] <== 9;
            rpForSolution[i][j].in <== solution[i][j];
            assert(rpForSolution[i][j].out == 1);
